...
8. [x] 로그인 게이트 (2026-07-27) — ADMIN role에 `LOGIN` 추가, 관리자 창 전체(카메라
       캘리·로봇 제어·로그 모니터)를 로그인 뒤로 이동. 캘리 결과는 "그 시점에 로그인된
       계정"에 영속 저장(로그인 없으면 세션에만 남고 서버 재시작 시 소실 — 이게 순서를
       강제한 이유). 브라우저별 세션 쿠키 + 로그아웃, 서버 링크 끊기면 전체 세션 무효화.

9. [x] **채널별 캘리브레이션** (2026-08-03, 프로토콜 v0.4) — 카메라가 4채널
       (PNM-C16083RVQ)이 되면서 채널마다 렌즈 방향이 달라 캘리브레이션 번들
       (K/D/H)이 완전히 다르다. 상단 바에 채널 선택을 두고, 캐시를 채널별로 나눠
       `H_MATRIX` 에 `ch` 를 실어 보낸다.
       - 채널을 바꾸면 `CMD SELECT_CHANNEL` 도 같이 나가 카메라가 그 채널을 본다.
         이걸 안 보내면 화면상 채널만 바뀌고 실제로는 계속 같은 채널을 캘리한다.
       - ⚠️ `ch` 를 안 실으면 서버가 전부 채널 1로 보고 **마지막에 캘리한 하나만
         남는다** — 에러는 안 나고 좌표만 틀린다. 회귀 테스트로 못박아 뒀다:
         `python3 calib_channel_test.py`
       - 채널 수는 `config.sh` 의 `RP_CAM_CHANNELS` (기본 4). `1` 로 두면 선택
         UI 가 사라지고 v0.3과 동일하게 동작한다 (PNO 단일 채널 현장).
...
#    브라우저: http://<파이IP>:8081        (카메라 캘리브레이션 대시보드, 로그인 필요)
#             http://<파이IP>:8081/robot  (로봇 제어 + 서버 로그)
#    포트는 기본값(6000/8081/6001) — 실제 배포 포트는 config.sh로 바뀔 수 있음
#    (이 리포 인스턴스는 6100/8083/6101 사용 — README.md 참고)
