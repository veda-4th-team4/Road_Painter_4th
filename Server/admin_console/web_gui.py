#!/usr/bin/env python3
"""Web GUI wrapper around pose_server.py's TCP harness.

Same protocol handling as pose_server.py (CAM_POSE / CALIB_* / LDC_CHECK_* /
LDC_SNAPSHOT), but instead of typing commands into a terminal, you click
buttons in a browser and watch the log stream live.

Snapshots and calibration views are received and written to disk (.ppm / .jpg
/ .json), but not displayed here — inspect the files directly.

Run:   python3 web_gui.py [tcp_port] [http_port] [snapshot_port]
       (defaults: tcp_port=6000, http_port=8081, snapshot_port=6001 —
        the ~/pos_receiver_eo instance; the team's ~/pos_receiver runs
        its own copy on 5000/8080/5001)
Then open http://<this-pi-ip>:8081 in a browser
  e.g. http://<lan-ip>:8081  or  http://<tailscale-ip>:8081 (Tailscale)

No external dependencies (stdlib only: http.server + Server-Sent Events).
"""
...
def tabbar(active):
    """Sticky top tab bar shared by all admin pages (lightweight integration)."""
    def cls(name):
...
<style>
.rp-tabbar{{position:sticky;top:0;z-index:99999;display:flex;gap:2px;
  background:#0d0d0d;border-bottom:1px solid #333;font-family:system-ui,sans-serif}}
.rp-tabbar a{{padding:10px 16px;color:#9ab;text-decoration:none;font-size:14px;
  border-bottom:2px solid transparent}}
.rp-tabbar a:hover{{background:#1a1a1a;color:#cde}}
.rp-tabbar a.active{{color:#fff;border-bottom-color:#6cf;background:#161616}}
</style>"""
...
<style>
  ...
  header a{color:#6cf;text-decoration:none;font-size:14px}
  h1{font-size:16px;margin:0;flex:1}
  main{padding:16px;max-width:720px;margin:0 auto}
  section{background:#1b1b1b;border:1px solid #333;border-radius:8px;
    padding:14px;margin-bottom:16px}
  h2{font-size:13px;text-transform:uppercase;letter-spacing:.05em;
    color:#9ab;margin:0 0 10px}
  button{background:#2a2a2a;color:#eee;border:1px solid #444;border-radius:6px;
    padding:12px 16px;font-size:15px;cursor:pointer;margin:4px}
  button:hover{background:#333}
  button:active{background:#3a3a3a}
  .estop{background:#5a1a1a;border-color:#a33}
  .estop:hover{background:#7a2020}
  .pad{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;max-width:280px}
  .pad .sp{visibility:hidden}
  #log{background:#0b0b0b;border:1px solid #333;border-radius:6px;height:240px;
    overflow:auto;padding:8px;font-family:ui-monospace,monospace;font-size:12px;
    white-space:pre-wrap;line-height:1.4}
  .tap{color:#8cf}.srv{color:#9d9}.warn{color:#fb6}
  .status{display:flex;gap:10px;flex-wrap:wrap;margin-bottom:4px}
  .chip{background:#0d0d0d;border:1px solid #333;border-radius:999px;
  ...
  .dot.on{background:#3c7}.dot.off{background:#c44}.dot.warn{background:#fb6}
</style></head><body>
<main>
  <section>
    <h2>상태</h2>
  ...
  <section>
    <h2>비상 / 재개</h2>
    <button class="estop" onclick="cmd('ESTOP')">■ 비상정지 (ESTOP)</button>
    <button onclick="cmd('RESUME')">▶ 재개 (RESUME)</button>
    <button onclick="cmd('CALIB_START')">◎ 캘리브레이션 시작</button>
  </section>
  <section>
    <h2>수동 조작 (조이스틱)</h2>
    <div class="pad">
      <span class="sp"></span>
      <button onclick="cmd('FORWARD')">▲ 전진</button>
      <span class="sp"></span>
      <button onclick="cmd('TURN_LEFT')">◀ 좌회전</button>
      <button onclick="cmd('STOP')">STOP</button>
      <button onclick="cmd('TURN_RIGHT')">우회전 ▶</button>
      <span class="sp"></span>
      <button onclick="cmd('BACKWARD')">▼ 후진</button>
      <span class="sp"></span>
    </div>
    <p style="color:#888;font-size:12px">
      ※ 관리자 명령은 경로 실행 중이어도 차단 없이 로봇에 전달됩니다.</p>
  </section>
  <section>
    <h2>실시간 로그 (서버 tap 포함)</h2>
    <div id="log"></div>
  </section>
</main>
<script>
...
function add(line){
  ...
  if(line.startsWith('[tap]'))d.className='tap';
  else if(line.startsWith('[server]'))d.className='srv';
  else if(line.startsWith('[!]'))d.className='warn';
  d.textContent=line;
  log.appendChild(d);
  while(log.childElementCount>500)log.removeChild(log.firstChild);
  log.scrollTop=log.scrollHeight;
}
// 서버 tap/상태 로그를 파싱해 상단 배너를 갱신한다.
...
# ([tap]/[server] lines) with a peer filter. Camera/other logs are hidden unless
# "전체 로그" is checked.
...
<style>
  ...
  .bar{display:flex;gap:14px;align-items:center;flex-wrap:wrap;margin-bottom:10px;
    font-size:14px}
  select,button{background:#2a2a2a;color:#eee;border:1px solid #444;border-radius:6px;
    padding:6px 10px;font-size:14px;cursor:pointer}
  #log{background:#0b0b0b;border:1px solid #333;border-radius:6px;
    height:calc(100vh - 130px);overflow:auto;padding:8px;
    font-family:ui-monospace,monospace;font-size:12px;white-space:pre-wrap;
    line-height:1.45}
  .tap{color:#8cf}.srv{color:#9d9}.warn{color:#fb6}.muted{color:#888}
</style></head><body>
<main>
  <div class="bar">
    <span>대상 필터:
    ...
    <button onclick="lines=[];render()">지우기</button>
...
function keep(l){
  if(!raw.checked && !l.startsWith('[tap]') && !l.startsWith('[server]')
     && !l.startsWith('[bridge]')) return false;
  const q=f.value; if(q && !l.includes(q)) return false;
  return true;
}
function cls(l){return l.startsWith('[tap]')?'tap':
  (l.startsWith('[server]')||l.startsWith('[bridge]'))?'srv':
  l.startsWith('[!]')?'warn':'muted';}
function render(){
  log.innerHTML='';
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ("/", "/robot", "/logs"):
            ...
            try:
                with open(HG_REFERENCE_PATH, "rb") as f:
                    body = f.read()
            except OSError:
            ...
            try:
                with open(HG_META_PATH, "rb") as f:
                    body = f.read()
            except OSError:
                body = b"{}"
            self.send_response(200)
            ...
            body = json.dumps(hg_experiment_status(), ensure_ascii=False).encode()
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
            self.send_header("Content-Length", str(len(body)))
            self.send_header("Cache-Control", "no-store")
            ...
            with hg_experiment_lock:
                latest = hg_experiment["last_export"]
            if not latest:
                self.send_response(404)
                self.end_headers()
                return
            try:
                with open(f"{HG_EXPERIMENT_DIR}/{latest['json']}", "rb") as f:
                    body = f.read()
            except OSError:
                self.send_response(404)
                self.end_headers()
    ...
    def do_POST(self):
        if self.path == "/cmd":
            ...
            try:
                ...
                raw = self.rfile.read(length) if length else b"{}"
                data = json.loads(raw)
                response = self.handle_hg_experiment_post(self.path, data)
    ...
    def handle_hg_experiment_post(self, path, data):
...
def main():
    ...
    threading.Thread(target=cctv_link_loop, daemon=True).start()
