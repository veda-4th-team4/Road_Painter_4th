...
새 `POS` 프레임을 **3장** 받은 뒤에 판정한다. 3장이 차기 전에 **1초**가 지나면
(마커 소실·CCTV 지연) 그냥 현재 pose로 판정한다.

지연 폭: `POS`가 정상(15~30Hz)이면 **100~200ms**. `POS`가 끊긴 경우 타임아웃
판정이 다음 수신 메시지에 얹혀 나가는데, 그때 남는 건 로봇 `STATUS`(500ms 주기)
뿐이라 **최악 1.5초**다.
...
- **로봇팀 영향 없음** — 프로토콜(메시지 형식·순서) 변경 아님. 로봇 입장에선
  ALIGN/GO 응답이 최대 1초까지 늦어질 수 있을 뿐이고, 그동안 정지 상태로
