...
`config/stream.json` 상태에 따라 서버가 셋 중 하나로 응답하고, QT는 셋을 각각
다르게 처리한다:

| 서버 설정 | `LOGIN_OK.stream` | QT 동작 |
|---|---|---|
| 파일 없음 / 파싱 실패 | **필드 없음** | "서버가 모른다" — 자기 QSettings 유지 |
| `enabled: false` | `{"enabled":false}` | 저장된 `relayBase`를 **해제**하고 카메라 직결 |
| `enabled: true` + `base` | `{"enabled":true,"base":…,"channels":…}` | 그 중계 주소 사용 |

- 🔴 **끌 때 필드를 생략하면 안 된다.** 생략은 "구버전 서버 / 설정 미확정"과
  구분되지 않아 QT가 저장된 중계 주소를 지울 근거가 없다. 그러면 중계를 내려도
  클라이언트가 죽은 `:8554`로 계속 접속을 시도하고, **PC마다 사람이 설정에서
  [비우기]를 눌러야 했다.** 그래서 "끔"도 명시적으로 내려보낸다.
- `enabled: false`일 때는 **`base`를 같이 보내지 않는다.** "끄라"는 지시에 주소가
  붙으면 QT가 어느 쪽을 따라야 할지 애매해진다.
- `enabled`는 켤 때도 명시적으로 싣지만, **키가 없으면 QT가 `true`로 읽는다** —
  이 플래그가 생기기 전 서버/파일과 그대로 호환된다.
- `enabled: true`인데 `base`가 비어 있으면 **안 싣는다** (주소를 모르는 상태).
- 파싱 실패한 파일은 "끔"이 아니라 **"모름"**으로 처리한다 — 깨진 파일 때문에
  QT의 멀쩡한 중계 주소를 지워버리면 안 된다.
- ⚠️ **`cam_ip`와 헷갈리지 말 것.** `cam_ip`는 카메라 IP(직결용)이고
  `stream.base`는 중계 서버 주소다. 합치면 직결로 되돌아갈 수 없게 된다.
- 파일은 **로그인마다 다시 읽는다** — 서버 재시작 없이 주소를 바꿀 수 있다.
- ⚠️ `enabled`는 **서버가 주소를 알려줄지**만 정한다. 중계 프로세스(MediaMTX)는
  `relay/start.sh`로 따로 띄워야 한다 — **중계를 쓰려면 둘 다** 필요하다.
