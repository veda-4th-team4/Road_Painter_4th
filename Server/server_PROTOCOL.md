...
> 최종 수정: 2026-08-04 · 구현 기준: `feature/pnm` 브랜치 (`Server/src/protocol.hpp`와 동일 내용)
>
...
> **2026-08-04 추가분: `LOGIN_OK.stream`(중계 RTSP 주소, 선택) + 정렬 수렴 수정
> (ALIGN 이후 READY 유예, 허용 오차 4°, POS 이상치/코너뒤집힘 게이트).
> 메시지 형식·순서 변경 없음 — 로봇/QT 코드 수정 없이도 종전대로 동작한다.**
...
| `CMD{"cmd":"SELECT_CHANNEL","ch":n}` | QT → 서버 → CCTV | 작업 채널 전환 |
...
- 채널이 바뀌면 서버는 **진행 중인 경로를 자동으로 폐기하지 않는다.** 작업 중
...
- 최대 반복 4회는 러프 디폴트 — 현장 튜닝 예정

#### 🔴 ALIGN 이후 READY는 서버가 늦게 답한다 (2026-08-03)

**`ALIGN`을 받고 회전한 뒤 보내는 `READY`에는 서버가 즉답하지 않는다.** 그 `READY`를 받은
시점부터 **새 `POS` 3장**이 도착한 뒤에 판정하고, 3장이 안 차면 **1초 뒤** 현재 pose로 판정한다.

왜냐면 미세회전은 0.1~0.3초짜리인데 그 회전이 CCTV → 서버 pose에 반영되는 데는 그보다
오래 걸린다. `POS`에 타임스탬프가 없어 서버는 받은 pose가 회전 전 장면인지 후 장면인지
구분할 수 없고, 그대로 판정하면 **회전 전 각도로 같은 `ALIGN`을 다시 쏴서 진동한다**
(과회전 → 반대 방향 ALIGN → 반복 → 4회를 태우고 정렬 안 된 채 `GO`).

로봇 입장에서 달라지는 것:

| | 응답 지연 |
|---|---|
| `POS` 정상 (15~30Hz) | 100~200ms |
| `POS` 끊김 | 최악 1.5초 (유예 1초 + STATUS 하트비트 주기 0.5초) |

- ⚠️ **`READY`에 자체 타임아웃을 걸어 임의로 출발하면 안 된다.** 서버 응답이 늦는 것은
  정상 동작이며, 기다리지 않고 출발하면 이 수정 전체가 무의미해진다.
- 유예 중 들어온 `POS`는 버리지 않고 **theta 평균**(sin/cos 누적)에 쓴다 — 어차피 기다리는
  시간이라 추가 지연 0으로 각도 노이즈가 1/√N로 준다.
- 프로토콜(메시지 형식/순서) 자체는 바뀌지 않았다 — 로봇 코드 수정은 필요 없다.

#### 허용 오차 2° → 4° (2026-08-03)

정렬 루프의 **1회 오차가 임계값과 같은 크기**라 수렴이 원리적으로 불가능했다.

1. 로봇 메인 루프가 80ms라 회전 중 1틱(= 400 SPS / 22.58스텝도 = **1.42°**)을 눈 감고 더 돈다.
   항상 목표를 지나치는 **한쪽 방향 계통 편향**이다.
2. 서버 `theta`는 마커 코너 픽셀에서 매 프레임 생으로 뽑아 평활화가 없다 — 마커 13cm 기준
   **코너 1px당 약 0.7°**.

제어 정밀도는 1회 오차의 3배는 잡아야 한다. **로봇이 감속 접근이나 예측 정지로 오버슛을
줄이면 다시 내릴 값이다.**

#### POS 이상치 게이트 (2026-08-03)

각도 변화가 **`3° + 40°/s × 경과시간`** 을 넘는 `POS` 프레임은 폐기한다 (연속 5회를 넘으면
그냥 받아들여 재동기 — 로봇을 들어 옮긴 경우에 영영 못 붙는 것을 막기 위해).

근거는 로봇의 물리적 최대 회전속도가 **17.8°/s**로 알려져 있다는 것이다
(TURN 400 SPS, 15433스텝/m, 축간거리 0.167m → 2×(400/15433)/0.167 = 0.31 rad/s).
그보다 훨씬 빠른 각도 변화는 주행이 아니라 **검출 오류**다 — 코너 순서가 한 칸 돌아가면
`theta`가 정확히 90°/180° 튀고, 부분 가림·반사로 코너 하나만 어긋나도 수십 도가 어긋난다.
지금까지 아무 검사가 없어 그런 프레임 하나면 엉뚱한 `ALIGN`/`DRIFT`가 나갔다.

허용치에는 **상한 45°** 가 있다 (2026-08-04 추가). 속도항만 두면 프레임 간격이 벌어질수록
게이트가 무한정 헐거워지는데, 현장 `POS`가 1~2Hz로 들어온 로그에서 간격 1.8초 = 허용 75°가
되어 90° 코너 뒤집힘이 통과 직전까지 갔다. 물리 상한 17.8°/s면 1.8초에 정당한 회전은 32°가
최대라, **45°를 넘는 변화는 간격이 아무리 길어도 검출 오류로 본다.**

#### 코너 순서 뒤집힘 게이트 (2026-08-04)

**각도 변화 > 60° 인데 중심 이동 < 3cm** 인 프레임은 별도로 폐기한다.

pose의 중심 `(x,y)`는 코너 4점의 **평균**이라 코너 순서가 한 칸 돌아가도 값이 변하지 않는다.
반면 `theta`는 앞변 중점 − 뒷변 중점으로 구해 정확히 90°씩 튄다. 그래서 **"각도는 크게
변했는데 중심은 제자리"는 물리적으로 불가능한 조합**이고(마커가 실제로 돌았다면 중심도 최소한
흔들린다), 코너 순서 회전의 지문이다. 속도 게이트와 별개로 두는 이유는 프레임 간격이 길어
속도 게이트를 통과해 버리는 플립을 잡기 위해서다.

> ⚠️ CCTV 팀: 이 게이트들은 **검출 오류를 조용히 덮는 것이 아니라 로그로 드러낸다.**
> `[WARN] POS 이상치` / `[WARN] POS 코너 순서 뒤집힘 의심` 이 반복되면 마커 검출 쪽을
> 봐야 한다는 신호다. 서버는 10초마다 아래 요약도 남긴다:
>
> ```
> [INFO] POS 10초 요약 - 수신 18, 채택 15 (1.5Hz), 파싱실패 0, 속도게이트 2, 코너뒤집힘 1
> ```

#### 유예 중 새 POS가 0장이면 GO (2026-08-04)

`ALIGN` 이후 유예가 끝났는데 **새 `POS`가 한 장도 안 들어왔으면 판정하지 않고 `GO`를 보낸다.**

그 상태로 판정하면 `pose_`가 직전 `ALIGN`을 계산할 때와 글자 그대로 같으므로 오차도 같고,
**똑같은 `ALIGN`이 한 번 더 나가는 것이 보장된다.** 2026-08-04 현장 로그에서 실제로 `-34.6°`가
값까지 동일하게 세 번 나갔고, 로봇은 그만큼 세 번 돌아 약 104°를 회전했다.

`GO`를 택한 이유: 남은 각도 오차는 주행 중 `DRIFT`가 이어서 잡아주지만, **중복 `ALIGN`이 만든
누적 오버슛은 되돌릴 방법이 없다.**

#### DRIFT 주기 200ms → 400ms (2026-08-04)

현장 `POS` 실측이 1~2Hz라 200ms(5Hz) 상한은 애초에 병목이 아니었다 — `POS`가 도착하는 족족
내보내던 것과 다를 바 없었다. 주기를 늘려도 응답성 손해는 없고, 로봇 쪽 로그/UART 부담과
`DRIFT` 스팸만 줄어든다.
...
> | **받아둔 경로** | **유지** — `RESUME` 하면 멈춘 세그먼트부터 **이어서 달림** | **폐기** — `RESUME` 해도 아무 데도 안 감 |
>
> 즉 `ESTOP`은 일시정지, `ABORT_DRAW`는 취소다. 로봇은 이걸 받으면:
> 1. 모터 정지 + 노즐 올림 + 비상정지 래치 (`ESTOP`과 동일)
> 2. **경로 버퍼를 비운다** — 세그먼트 커서, `PATH_DONE` 전송 대기, 노즐 오프셋
>    서브시퀀스 상태까지 전부 초기화. `RESUME` 후에 자동 주행이 되살아나면 안 된다.
...
| `LOGIN_OK` | `{"id":"user1","calib":{...}\|null,"calibs":{"1":{...},"2":null,…},"active_ch":1,"cam_ip":"192.168.0.31"\|null,"stream":{"base":"rtsp://192.168.0.2:8554","channels":4}}` |
...
| 필드 | 설명 |
|---|---|
| `calib` | **활성 채널**의 캘리브레이션 번들 (**`null`이면 그 채널은 캘리브레이션 필요**). v0.3과 의미가 같아 옛 클라이언트도 그대로 동작한다 |
| `calibs` | **채널별 번들 맵** (2026-08-03 신설). 키는 채널 번호 문자열, 값은 번들 또는 `null`. 4채널 UI가 "어느 채널이 캘리 됐는지"를 한눈에 표시하는 데 쓴다 |
| `active_ch` | 서버가 기억하는 활성 채널 (기본 1) |
| `cam_ip` | 등록해둔 카메라 IP (없으면 `null`) |
| `stream` | 중계 서버 RTSP 베이스 주소와 채널 수 (2026-08-04 신설, **선택**). **필드가 없으면** QT가 로컬 설정값을 쓴다 |

#### `stream` — 중계 RTSP 주소 (2026-08-04 신설, 선택)

카메라가 1채널(PNO)에서 4채널(PNM-C16083RVQ)로 바뀌면서 RTSP를 카메라 직결이 아니라
**중계 서버(MediaMTX)를 거쳐** 받는다. 그 주소를 사용자가 설정 화면에 손으로 입력하면
주소가 바뀔 때마다 **모든 사용자가 각자 다시 입력**해야 하고 오타 하나로 영상이 끊긴다.
로그인 때 서버가 알려주면 그 문제가 없어진다.
...
| 필드 | 값 |
|---|---|
| `base` | 중계 서버 RTSP 베이스 주소. **끝에 `/` 없음** (서버가 붙어 있으면 떼고 보낸다) |
| `channels` | 채널 수. **값이 없으면 생략된다** — QT가 자기 기본값(4)을 쓴다 |

QT는 이 베이스로 채널 URL을 규칙에 따라 조립한다 — 서버는 베이스만 준다:

```
메인  {base}/ch1 … /ch4      (2592x1520 30fps)
서브  {base}/ch1s … /ch4s    (640x480 10fps, 2x2 미리보기용)
```

**서버가 이 필드를 안 보내면 아무 일도 일어나지 않는다.** QT는 종전대로 로컬 설정값을 쓴다.

| 상황 | QT 동작 |
|---|---|
| `stream` 없음 | 기존과 100% 동일 — 설정에 저장된 값 사용 |
| `stream.base` 있음 | 그 값으로 자동 설정 + 로그 기록 |

> ⚠️ **`cam_ip`와 혼동하지 말 것.** `cam_ip`는 *카메라 IP*이고 QT는 그것으로 PNO 단일채널
> **직결** URL을 조립한다. `stream.base`는 *중계 서버 주소*다. 둘을 한 필드로 합치면
> **PNO로 되돌아갈 수 없게 된다** — 4채널 전환은 아직 시도 단계라 언제든 되돌릴 수 있어야 한다.

##### 서버 설정 방법

`config/stream.json` 을 두면 그 값이 실린다. **파일이 없으면 필드를 싣지 않는다**(기본값) —
중계를 안 쓰는 현장(PNO 직결)은 이 파일을 만들지 않으면 그만이다.

```bash
cp config/stream.json.example config/stream.json   # 그 뒤 base 를 실제 주소로 수정
```

- IP는 **QT 클라이언트에서 닿는** 중계 서버 주소여야 한다. 서버는 `INADDR_ANY`로 떠 있어
  자기 주소를 알 수 없으므로 자동으로 채울 방법이 없다.
- 포트는 `relay/cameras.env` 의 `MTX_RTSPADDRESS`(기본 `:8554`)와 같아야 한다.
- **로그인할 때마다 다시 읽는다.** 주소를 바꾸려고 서버를 재시작하면 로봇/CCTV 연결이
  통째로 끊기므로, 파일만 고치고 QT가 재로그인하면 새 값이 내려간다.

확인은 서버 로그로 한다 — 로그인할 때마다 현재 값이 찍힌다:

```
[INFO] LOGIN user1 성공 (QT 요청, 캘리브레이션 4채널 보유, 활성 채널 1 전달, 중계 주소 rtsp://192.168.0.2:8554)
[INFO] LOGIN user1 성공 (QT 요청, …, 중계 주소 없음 - QT 설정값 사용)
```
...
- **작업 완전 중지: `"ABORT_DRAW"`** (2026-08-03 신설) — "작업 중단" 버튼.

  ```json
  {"type":"CMD","seq":9,"payload":{"cmd":"ABORT_DRAW"}}
  ```

  진행 중인 작업을 **취소**한다. 서버는 경로 실행 상태(진행 커서·활성 세그먼트·
  접근 대기·시작 요청)를 전부 버리고, 같은 명령을 **로봇에 중계**해 로봇도 받아둔
  `PATH`를 버리게 한다 (로봇 동작은 "로봇 (ROBOT)" 장 `ABORT_DRAW` 절 참고 —
  정지 + 비상정지 래치 + 경로 폐기).
  - 응답: `DRAW_ABORTED {"was_active":true|false}` — 실제로 진행 중이던 작업을
    껐는지. `false`면 이미 아무것도 안 돌고 있었다는 뜻이라 UI는 조용히 정리만 하면 된다.
  - **도면(`BLUEPRINT`)은 지우지 않는다.** 취소 후 `START_DRAW`를 다시 누르면
    같은 도면으로 처음부터 다시 시작한다 (도면을 새로 올릴 필요 없음).
  - 로봇은 비상정지 래치가 걸린 채로 남으므로 **다시 움직이려면 `RESUME`이 필요하다.**
    다만 경로는 이미 버려졌으므로 `RESUME`을 해도 도색이 재개되지는 않는다.

  > 🔴 **왜 만들었나**: v0.3까지는 "작업 중단"이 `ESTOP` 하나뿐이었는데, `ESTOP`은
  > 일시정지라 `RESUME`을 누르면 로봇이 **멈춘 지점부터 도색을 이어서 재개**했다.
  > 서버 쪽 경로 상태도 남아 있어 이후 `START_DRAW`가 `DRAW_FAIL{busy}`로 거절됐다.
  > 즉 **한 번 시작한 작업을 취소할 방법이 프로토콜에 없었다.**

- **작업 채널 전환: `"SELECT_CHANNEL"`** (2026-08-03 신설, 다채널 카메라 전용)

  ```json
  {"type":"CMD","seq":10,"payload":{"cmd":"SELECT_CHANNEL","ch":2}}
  ```

  4채널 미리보기에서 사용자가 작업할 채널을 고르면 보낸다. 서버는 활성 채널을
  바꾸고 **`CCTV`에 그대로 중계**한다 (로봇에는 안 보낸다 — 로봇과 무관).
  - 응답: `CHANNEL_OK {"ch":2, "calib":{...}|null}` — 그 채널의 캘리브레이션 번들.
    QT는 이걸 받아 top-view를 그 채널 기준으로 다시 만든다.
  - `calib`이 `null`이면 **그 채널은 아직 캘리브레이션이 안 됐다.** 도면 좌표를
    믿을 수 없으므로 QT는 작도를 막고 관리자 창 안내를 띄운다.
  - `ch`가 없거나 범위 밖이면 `CHANNEL_FAIL {"reason":"bad_channel"}`.
  - ⚠️ **경로 실행 중에는 채널을 바꾸지 말 것.** 서버는 막지 않지만, 지금 로봇을
    보고 있는 채널을 바꾸면 pose 공급이 끊긴다. QT가 작업 중 채널 전환 UI를 잠근다.
...
- ⚠️ **경로 실행 중에는 수동 조작이 차단된다** (2026-07-21 추가): 서버가 PATH를 보내
  ...
  `ABORT_DRAW` 같은 비수동 명령은 항상 통과한다 (막히면 위험하거나, 막으면
  작업을 취소할 방법이 없어진다). 현재 거절 응답 메시지는 없으며
...
- `POSE`: 서버가 POS를 변환해 계산한 로봇 pose — **top-view 위 로봇 표시용**
  ...
  - **발행 주기 = CCTV `POS` 주기 그대로.** 서버는 `POSE`를 솎지 않는다 (`POS` 1건당 `POSE` 1건, 스로틀 없음 — 같은 핸들러의 `DRIFT`를 5Hz로 제한하는 것과 다르다). 현장 관측 15~30Hz이며 카메라 처리 속도에 따라 변한다 — **QT는 상한을 가정하지 말 것.**
...
- `CHANNEL_OK` / `CHANNEL_FAIL`: **작업 채널 전환 결과** (2026-08-03 추가)

  ```json
  {"type":"CHANNEL_OK","seq":22,"payload":{"ch":2,"calib":{...}}}
  {"type":"CHANNEL_FAIL","seq":22,"payload":{"reason":"bad_channel"}}
  ```
  - `CMD{"cmd":"SELECT_CHANNEL"}`에 대한 응답. `calib`은 그 채널의 번들이며
    **`null`이면 그 채널은 아직 캘리브레이션이 안 된 것**이다.
  - QT는 이 `calib`으로 top-view를 다시 만든다 — `H_MATRIX`를 받았을 때와 **같은
    경로**를 타야 한다 (K/D 반영, `coord_mode` 판정까지).
- `DRAW_ABORTED`: **작업 취소 확인** (2026-08-03 추가)

  ```json
  {"type":"DRAW_ABORTED","seq":23,"payload":{"was_active":true}}
  ```
  - `CMD{"cmd":"ABORT_DRAW"}`를 받아 서버가 경로 상태를 버린 직후 1회 전송.
  - `was_active`가 `false`면 진행 중인 작업이 없었다는 뜻 (버튼 연타 등). QT는
    어느 쪽이든 "그리는 중" 표시를 끄면 된다.
  - ⚠️ **QT는 이 응답을 기다리지 말고 UI를 먼저 정리해도 된다.** 취소는 사용자가
    급할 때 누르는 버튼이라, 서버 왕복을 기다리며 화면이 멈추면 안 된다.
...
`{"cmd":"SELECT_CHANNEL","ch":2}` — **작업 채널 전환** (2026-08-03 신설, **CCTV팀 신규 구현 대상**)

- 다채널 카메라에서 "지금 로봇을 봐야 할 채널"이 바뀌었음을 알린다. 카메라 앱은
  그 채널의 영상으로 검출 대상을 바꾸고, 이후 `POS`에 그 `ch`를 실어 보낸다.
- **단일 채널 카메라(PNO)는 이 명령을 무시해도 된다.** 서버는 응답을 기대하지 않는다.
- ⚠️ 현재 카메라 앱(`CCTV/src/central_tls_sender.cpp`)의 수신 파서는
  `"cmd":"CALIB_START"` 문자열 하나만 인식한다. `SELECT_CHANNEL`을 받으려면
  그 파서를 고쳐야 한다.
...
| 필드 | 설명 |
|---|---|
| `ch` | **이 번들이 어느 채널의 것인지** (2026-08-03 추가). `payload` 최상위에 붙는다 — `calib` **안이 아니다**. 생략하면 채널 1 |
...
- 🔵 **`ch`는 세 스키마 모두 `payload` 최상위에서 읽는다** (2026-08-03). 평면
  스키마는 `payload` 자체가 번들이라 `ch`가 번들 안에 남는데, 서버는 그것을
  **`calib_id`처럼 손대지 않는 메타데이터로 보존**한다 — 저장된 번들만 봐도 어느
  채널 것인지 알 수 있어 오히려 낫다. 중첩 형식은 `ch`를 `calib` 바깥에 둔다.
...
🔵 **2026-08-03: 두 저장소 모두 채널별 맵이 됐다** (`{"1":{…},"2":{…}}`). 저장·조회는
**채널 단위로 독립**이다 — 채널 2를 캘리해도 채널 1의 번들은 그대로 남는다.
예전 형식(맵이 아닌 번들 하나)이 파일에 있으면 **채널 1의 번들로 읽는다.**

- `LOGIN_OK` 조회 순서: **계정 값 → (없으면) 전역 값**. 계정에 고정해둔 번들을 전역이 덮지 않는다. (채널마다 각각 판정)
...
```json
{"type":"POS","seq":10,"payload":{"ch":2,"corners":[[u1,v1],[u2,v2],[u3,v3],[u4,v4]]}}
```
- `corners`: 로봇 마커 4점 = **원본 CCTV 픽셀 좌표**, 순서 = **[전좌, 전우, 후우, 후좌]**
- `ch`: **이 마커를 본 채널** (2026-08-03 추가, 선택 — 생략하면 1). 서버는 이 채널의
  캘리브레이션(K/D/H_marker)으로 좌표를 변환한다. **틀린 채널을 실으면 좌표가 통째로
  어긋난다** — 에러 없이 그럴듯한 값이 나오므로 특히 주의할 것.
  - 서버의 **활성 채널과 다른 `ch`가 오면 그 `POS`는 무시된다** (위 "채널 규약" 참고).
...
| `calibs` / `active_ch` 필드 | `LOGIN_OK` | 서버 → QT |
| `CMD "SELECT_CHANNEL"` + `CHANNEL_OK`/`CHANNEL_FAIL` | 채널 전환 | QT → 서버 → CCTV |
...
로봇이 멈춘 세그먼트부터 도색을 **이어서 재개**했고, 서버에도 경로 상태가 남아
...
1. **`CMD{"cmd":"ABORT_DRAW"}` 처리 (신규)**: `ESTOP`과 똑같이 정지·노즐 올림·
   비상정지 래치를 하되, **받아둔 `PATH`를 버린다.** 세그먼트 커서·`PATH_DONE` 전송
   대기·노즐 오프셋 서브시퀀스까지 전부 초기화해서, **`RESUME` 후에 자동 주행이
   되살아나지 않게** 한다. `PATH_DONE`은 보내지 않는다 (끝낸 게 아니라 버린 것).
   자세한 표는 "로봇 (ROBOT)" 장 `수신: CMD` 절 참고.
   - 그 외 변경 없음. `ch`는 로봇과 무관하다 (로봇은 좌표를 모른다).

**QT팀** — 4개
1. **4채널 미리보기 → 채널 선택 → `SELECT_CHANNEL` 전송 → `CHANNEL_OK.calib`으로
   top-view 재구성.** `calib`이 `null`인 채널은 작도를 막을 것.
2. **`LOGIN_OK.calibs`로 채널별 캘리 여부 표시** (어느 채널이 준비됐는지 한눈에).
   기존 `LOGIN_OK.calib`은 활성 채널 번들로 계속 오므로 단일 채널 경로는 그대로 둔다.
3. **"작업 중단" 버튼이 `ABORT_DRAW`를 보내도록 변경.** 지금은 `ESTOP`만 보내서
   서버·로봇 상태가 남는다.
4. **경로 실행 중에는 채널 전환 UI를 잠글 것** (서버는 막지 않는다).

**서버팀** — 구현 완료 대상 (이 변경의 주체)
- `calib_` 단일 필드 → 채널별 맵 + `activeChannel_`
- `H_MATRIX`/`POS`의 `ch` 처리, 비활성 채널 `POS` 무시
- `SELECT_CHANNEL` → CCTV 중계 + `CHANNEL_OK` 회신
...
| **`READY`** | 로봇 → 서버 | **출발 전 정렬 확인 요청**: TURN을 마치고 MOVE를 시작하기 직전, 정지 상태에서 곧 실행할 MOVE의 인덱스(`seg`)를 담아 전송한다. 서버는 CCTV 피드백으로 실제 각도를 확인해 응답할 때까지 로봇은 대기한다. **`ALIGN` 직후의 READY에는 서버가 의도적으로 늦게 답한다**(최대 1.5초) — 자체 타임아웃으로 임의 출발 금지. |
| **`ALIGN`** | 서버 → 로봇 | **출발 전 미세 각도 보정**: READY 시점의 실제 각도와 목표 `heading_deg`의 오차가 허용치(4°, 2026-08-03 2°에서 상향)를 넘으면 보정할 회전량(`angle_deg`, 양수=좌회전)을 보낸다. 로봇은 그만큼 회전 후 다시 READY를 보낸다. |
