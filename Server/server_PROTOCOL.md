...
{"type":"BLUEPRINT","seq":4,"payload":{
  ...
  "pen_offset_m": 0.04,
  "program": [
    {"op":"MOVE","v":0,"dist_m":0.04,"paint":false,"heading_deg":90.0,"speed_mps":0.25,"pivot":true},
    {"op":"NOZZLE","v":0,"down":true},
    {"op":"MOVE","v":0,"dist_m":3.0,"paint":true,"heading_deg":90.0,"speed_mps":0.08},
    {"op":"NOZZLE","v":1,"down":false,"pivot":true},
    {"op":"MOVE","v":1,"dist_m":-0.04,"paint":false,"heading_deg":90.0,"pivot":true},
    {"op":"TURN","v":1,"angle_deg":-90.0,"heading_deg":0.0,"speed_dps":20.0,"pivot":true},
    {"op":"MOVE","v":1,"dist_m":0.04,"paint":false,"heading_deg":0.0,"pivot":true},
    {"op":"MOVE","v":1,"dist_m":0.5,"paint":false,"heading_deg":0.0,"speed_mps":0.25}
  ]}}
```
...
| `pen_offset_m` | 선택 | 펜(노즐)이 로봇 회전중심 **뒤**로 떨어진 거리 `d` |
...
**선택 필드 3개는 전부 하위호환** — 안 보내면 종전과 100% 동일하게 서버가 경로를 생성한다.

- `paint` 길이가 `points`와 다르면 → 도면은 살리고 이 필드만 무시 (= 전 구간 도색)
- `program`이 없거나 형식이 깨지면 → 서버가 예전처럼 `points`로 직접 생성

### `program` op 규약 (2026-07-28 신설)

Qt가 만들고 서버는 손대지 않고 중계한다. **로봇이 그대로 실행할 동작 목록**이다.

| op | 필드 | 의미 |
|---|---|---|
| `MOVE` | `dist_m` | 직진. **음수 = 후진** (바라보는 방향은 안 바뀜) |
| | `paint` | 이 구간을 칠하는가 |
| | `speed_mps` | 이 구간의 주행 속도 — **도색 구간과 이동 구간이 다르다** |
| `TURN` | `angle_deg` | 제자리 회전. **양수 = 좌회전** (종전과 동일) |
| | `speed_dps` | 제자리 회전 속도 (deg/s) |
| `NOZZLE` | `down` | 노즐 내림(`true`) / 올림(`false`) |
| 공통 | `heading_deg` | 이 동작 후 로봇이 **바라보는** 절대 방위 |
| 공통 | `v` | 이 op가 담당하는 도면 꼭짓점 index (이탈 복귀 재개 지점 계산용) |
| 공통 | `pivot` | 꼭짓점 방향전환 동작인가 (아래). 없으면 `false` |
...
펜이 회전중심에서 `d`만큼 뒤에 있어, 제자리 회전하면 펜이 **반지름 `d`짜리 호**를 그려 바닥에 자국이 남는다 (5cm·90° = 7.9cm). 그래서 꼭짓점마다 이 순서로 돈다:

```
노즐 올림 → 후진 d → 제자리 회전 Δθ → 전진 d → 노즐 내림
```

후진량과 재전진량이 **정확히 `d`** 여야 회전축이 곧 꼭짓점이 되고 펜이 제자리로 복귀한다. 이 op들에 `pivot:true`가 붙는다.

- ⚠️ **노즐이 이미 올라가 있어도(도형 사이 이동) 후진/회전/전진을 건너뛸 수 없다.** 건너뛰면 다음 직진의 시작점이 `d·(ĥ₂−ĥ₁)`만큼 어긋난다.
- 🔴 **서버는 `pivot` 구간에서 `ALIGN`/`DRIFT`를 보내지 않는다.** 각도를 보정하면 후진 방향이 틀어져 "후진량 = 재전진량"이 깨지기 때문. `READY`가 오면 판정 없이 곧바로 `GO`를 준다.
- **시작 lead-in**: 서버는 로봇 **중심**을 `points[0]`에 세우기만 한다. 그 순간 펜은 `d`만큼 뒤에 있으므로 `program` 맨 앞에 `MOVE +d (paint=false)`가 들어간다. ⚠️ 서버가 접근 목표를 `points[0] + d`로 밀면 **이중 적용**된다 — 접근 목표는 `points[0]` 그대로다.
...
- `BLUEPRINT_OK`: **도면 접수 확인** (2026-07-28 추가)
  ...
  {"type":"BLUEPRINT_OK","seq":21,"payload":{
    "points": 4, "paint": true, "program": 8, "pen_offset_m": 0.04}}
  ```
...
- 서버 동작:
  ...
  2. **지금 달리고 있는 구간**에서 **0.3 m 초과 이탈** 시 복귀 PATH를 로봇에 재전송 (최소 3초 간격) — 임계값은 러프 디폴트, 현장 튜닝 예정
     > 2026-07-28 변경: 예전에는 "도면 **전체**에서 가장 가까운 꼭짓점"으로 재계획했다. 횡단보도처럼 나란한 줄이 여러 개면 3번 줄을 칠하다 밀린 로봇에게는 4번 줄 꼭짓점이 제일 가까워, **재계획 한 번에 3번 줄을 버리고 4번 줄로 건너뛰었다.** 같은 이유로 옆줄 위에 올라타면 "경로 위"로 판정돼 이탈을 아예 못 잡았다. 지금은 서버가 진행 커서를 들고 "원래 향하던 꼭짓점"으로 복귀시킨다. `program`이 있으면 **복귀 구간만 새로 만들고 원본 시퀀스를 잘라 이어 붙인다** (재생성하면 꼭짓점 동작이 사라져 펜 위치가 어긋난다).
...
| 속도 | 로봇 펌웨어 고정값 | op별 `speed_mps` / `speed_dps` |

**왜**: 꼭짓점 동작은 **펜 오프셋을 아는 쪽**이 만들어야 하고, 경로는 한 번 정해지면 불변이라 서버가 실시간으로 만들 이유가 없다. 무엇보다 **Qt 화면의 "로봇 동작 시퀀스" 미리보기가 곧 실행본**이 되어, 미리보기와 실제 주행이 어긋나 조작자가 검증할 수 없던 문제가 사라진다.
...
1. **`BLUEPRINT`에 선택 필드 3개 추가** — `paint[]`(구간별 도색 여부, 한붓그리기 → 여러 획),
   `pen_offset_m`(펜이 회전중심 뒤로 떨어진 거리), `program[]`(동작 시퀀스).
   **전부 선택이라 안 보내면 종전과 100% 동일하게 동작한다.**
2. **`BLUEPRINT_OK` 신설** — 도면을 받은 즉시 개수를 회신해 Qt가 대조할 수 있게 한다.
3. **`program` op 신설** — `NOZZLE`, `MOVE` 음수(후진), op별 속도. `heading_deg`가
   "진행 방향" → **"바라보는 방위"** 로 의미가 바뀌었다 (후진 op에서 180° 차이).
4. **꼭짓점 동작(`pivot`) 구간에서 `ALIGN`/`DRIFT` 차단** — 각도를 보정하면
   "후진량 = 재전진량 = d"가 깨져 펜이 꼭짓점으로 돌아오지 못한다. `READY`엔 곧바로 `GO`.
5. **이탈 판정/복귀를 진행 커서 기준으로 교체** — 옆줄로 튀던 문제 수정 (CCTV `POS` 절 참고).

**팀별 반영**

- **QT팀**: 위 3개 필드를 실어 보내면 된다. `BLUEPRINT_OK`로 접수 확인 가능.
- **로봇팀**: 신규 처리 4개 — `NOZZLE {down}` op, `MOVE.dist_m` **음수 = 후진**,
  `MOVE.speed_mps`/`TURN.speed_dps`, `CMD SET_SPEED {travel_mps,paint_mps,turn_dps}`.
