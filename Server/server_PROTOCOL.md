...
- **2026-08-07 추가 — `enabled` 플래그**: `{"enabled": false}`면 `base`가 적혀
  있어도 이 필드를 **싣지 않는다**(= 직결 모드). 중계를 끌 때 파일을 지우면 다시
  켤 때 주소를 잃으므로, `base`는 남기고 플래그만 뒤집는다. 키가 없으면 `true`로
  읽어 기존 파일이 그대로 동작한다.
  ⚠️ 이 플래그는 **서버가 주소를 알려줄지**만 정한다. 중계 프로세스(MediaMTX)는
  `relay/start.sh`로 따로 띄워야 한다 — **중계를 쓰려면 둘 다** 필요하다.
- 🔴 **미해결**: 서버가 중계를 꺼도 QT는 QSettings에 저장된 `relayBase`를 계속
  쓴다("안 보냄"을 "그대로 둬라"로 해석하는 현재 규약). 지금은 QT 설정 화면에서
  사람이 [비우기]를 눌러야 직결로 넘어간다. 해소 방안은
  [`docs/QT_ACTION_ITEMS_20260807.md`](docs/QT_ACTION_ITEMS_20260807.md) Q-2 참고.
