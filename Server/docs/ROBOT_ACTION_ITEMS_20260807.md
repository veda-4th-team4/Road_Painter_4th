# 로봇팀 전달 사항 (2026-08-07) — ✅ **전건 종결**

**대조 기준**: 로봇 `feature/server-driven-v2` @ `67b8349` (2026-08-07 13:55)
**정본 규격**: `PROTOCOL_v2_ROBOT.md`

> ## ✅ 이 문서의 모든 항목이 닫혔습니다 (2026-08-07)
>
> A-1 ~ A-5 다섯 건 전부 코드에서 확인했고, 그 뒤 2차로 올린 `UpdateArc()` 완주 판정
> 문제와 `kMinArcRadius` 조정까지 `67b8349`에 반영됐습니다. **로봇 쪽 미결 항목 없습니다.**
>
> **최소 도색 반지름 = `R_paint ≥ 0.200m`** 로 확정(로봇팀 회신). 서버
> `min_paint_radius_m` 기본값에 반영했고, 이 값이 Qt팀에 전달됩니다.
>
> 이력: `d3b6ec1`(A-1) → `a58eacb`(A-2~A-5) → `67b8349`(완주 판정 + `kMinArcRadius`)
...
| # | 항목 | 반영 커밋 | 확인 |
|---|---|---|---|
| A-1 | `arc` 상태 가드 | `d3b6ec1` | ✅ `IsArc()` + `main.cpp:327` 가드. 요청 범위 밖까지 처리해 주셨음 |
| A-2 | `ESTOP`이 경로를 지움 | `a58eacb` | ✅ 6줄 제거. `ESTOP → RESUME` 재개 경로 복원 |
| A-3 | `path_done_sent` | `a58eacb` | ✅ `true`로 변경. 거짓 `PATH_DONE` 차단 |
| A-4 | `STOP` 분리 | `a58eacb` | ✅ 별도 분기로. 경로 유지 |
| A-5 | 0으로 나눗셈 | `a58eacb` | ✅ `kMinArcRadius` 분기 |
| 2차-1 | `UpdateArc()` 완주 판정이 `OR` | `67b8349` | ✅ `arc_is_left`로 바깥 바퀴 기준 판정 |
| 2차-2 | 실행 가능한 최소 `radius_m` | (회신) | ✅ **0.200m 확정** (마진 하한 0.180) |
| 2차-3 | `kMinArcRadius` `1e-3`→`2e-3` | `67b8349` | ✅ |

### 하한을 결정한 것은 역회전이 아니라 SPS 발산이었습니다

역회전(`R_paint < 0.176m`)은 **제약이 아니었습니다** — 모터 드라이버가 좌우 반대방향
동시 구동을 지원한다는 회신을 받았습니다. 실제 하한은 바깥 바퀴 속도가 `1/R_robot`로
발산하는 데서 나왔습니다. NEMA 17(3200 steps/rev) 기준 2000 SPS 초과 시 탈조 위험이고,
`R_paint = 0.200`이 1278 SPS로 여유가 있습니다.

확정된 0.200이 0.176보다 크므로 역회전 구간은 도면 단계에서 자연히 걸러집니다.

---

<details>
<summary>이하 원본 요청 내용 (기록 보존용 — 조치 완료)</summary>
...
| 항목 | 담당 | 상태 |
|---|---|---|
| `min_paint_radius_m = 0.200` 반영 | 서버 | ✅ 완료 (`params.hpp` / `params.json` / `PROTOCOL_v2_ROBOT.md` §5.5) |
| Qt에 최소 반지름 0.200 전달 | 서버 → Qt | 3차 회신서로 진행 |
| Qt `heading_deg` 진입 접선 push | Qt | 대기 중 — 이것이 들어와야 부분 원호 통합 시험 가능 |
| `LOGIN_OK`에 `min_paint_radius_m` 싣기 (§7.6) | 서버 | 미착수. Qt 하드코딩 제거용 |

**호 도색 통합 시험은 Qt push만 확인되면 잡을 수 있습니다.** 로봇 쪽 준비는 끝났습니다.
