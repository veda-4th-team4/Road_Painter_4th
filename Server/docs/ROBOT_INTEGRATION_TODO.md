...
> ⚠️ **미해결 설계 분기 먼저 확인**: SRS v1.6은 "로봇이 POSE(x,y,θ)를 받아 횡방향/헤딩
> 오차를 직접 계산해 추종"하는 구조였지만, 현재 서버 구현(v0.3)은 **로봇에 좌표를 주지
> 않고 동작 시퀀스(TURN/MOVE) + 각도 피드백(ALIGN/DRIFT)만** 보냅니다. 아래 3~4번은
> 후자(v0.3) 기준입니다. **로봇 제어를 어느 방식으로 갈지 먼저 팀 간 확정**이 필요하며,
> 확정 결과에 따라 이 문서와 `Server/PROTOCOL.md`, SRS를 정합화합니다.
