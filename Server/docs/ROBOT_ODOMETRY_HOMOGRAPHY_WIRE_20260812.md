...
### 5-1. 폐합오차 — 서버 로그 전용 진단 (2026-08-13 보강)

`idx 0`(출발 자리, 아직 한 발도 안 움직인 상태)과 `idx 8`(한 바퀴 돌고 돌아온
자리)은 **같은 물리 지점**이다(`odoPointWorldMm`의 `case 0`과 `default`가 같은
좌표). 로봇이 정확히 복귀했다면 두 픽셀이 같아야 하고, 차이가 곧 한 바퀴 누적된
주행 오차다. **H가 없어도 성립하는 유일한 자기검증**이다.

```
[INFO] 오도메트리 폐합오차 [채널 2] 15.0px = 약 30mm (둘레 3000mm의 1.00%)
       | 스케일 0.500px/mm (4변 평균) | idx0=(300.0,1200.0) idx8=(315.0,1200.0)
```

원문은 "**mm 환산 불가** — 그 시점엔 새 캘리가 없다"였는데, 엄밀한 환산은 지금도
불가능한 게 맞다. 다만 **거친 스케일은 측정값 자체에서 나온다**: 네 변의 실측
픽셀 길이를 명령 mm로 나눠 평균하면 px/mm가 얻어진다. 픽셀 값만으로는 "34px가
심각한가"를 알 수 없어 로그가 사실상 안 읽혔기 때문에, 둘레 대비 %까지 같이
남긴다. mm 환산본의 정본은 여전히 카메라 기록이다.

**🔴 진단 전용이다. 서버는 이 값으로 아무 판정도 하지 않는다** — 세션을
실패시키지도, Qt에 통지하지도 않는다. 유효점 판정과 H 산출은 카메라 몫이고(§7)
서버가 뒤집으면 권한이 겹친다. 관리자 창은 서버 로그를 `LOG`로 중계받으므로
(`tls_server.cpp`) 별도 전송 없이 이 줄을 그대로 본다.

**이 값의 한계 — 읽는 사람이 반드시 알아야 한다:**

| 한계 | 뜻 |
|---|---|
| **위치만 본다** | 제자리에 정확히 돌아왔어도 로봇이 엉뚱한 방향을 보고 있을 수 있다. 마커 중심 하나만 비교하므로 자세 오차는 안 잡힌다 |
| **오차가 상쇄된다** | 세 코너가 같은 쪽으로 오버슛하면 사각형이 작게 닫히면서 오히려 출발점 근처로 돌아온다. **작다고 정확한 게 아니다.** 반대는 성립한다 — 크면 확실히 틀렸다 |
| **스케일이 거칠다** | 카메라가 기울어져 있으면 화면 위치마다 실제 px/mm가 다르다. 자릿수를 보는 용도지 정밀 측정이 아니다 |

`idx 0` 또는 `idx 8` 캡처가 실패하면 "측정 불가"로, 코너 캡처가 부족해 스케일을
못 뽑으면 픽셀 값만 남긴다.
