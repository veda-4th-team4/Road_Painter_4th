...
2. **직선↔호 접합부 꺾임** (§5.6). `R_paint=0.5m`에서 약 18°. 실사용 반지름 확정 후
   (a) 감수 / (b) 전이 구간 삽입 중 선택.
3. **`R_paint` 실사용 하한**. 이론 하한은 0.155m지만 그 근처는 `R_robot → 0`이라
...
4. **수동 조작(`manualMode_`) 중 boundary 처리**. v1은 수동 CMD가 오면 자동 판정을 멈췄다.
   v2에서는 로봇이 READY로 대기 중일 수 있으므로, 수동 진입 시 대기 중인 READY를
   어떻게 회수할지 정해야 한다 (현재 안: 즉시 `GO`로 흘려보내지 말고 경로 자체를 폐기).
5. **꼭짓점당 실행 시간**. 노즐 op 2개(≈2초) + boundary 대기 2~4초 + 오프셋 이동 2회.
