...
#### 6.5.1 도색 언더슛 (2026-08-24 신설, `paint_undershoot_m`)
...
- 적용 대상은 **도색 직선뿐**이다.
  - `arc`는 제외 — `MORE`가 호 끝에 **접선 직선**을 덧붙이는 방식이라(§6.3),
    덜 그으면 원이 그만큼 찌그러진다.
...
| ALIGN 최대 횟수 | `6` | |
| MORE 불감대 | `0.005 m` | `more_deadband_m`. 2026-08-24 확인 - 표에 `0.03`으로 적혀 있었으나 실제 설정은 `0.005` |
| MORE 상한 (판정 폐기) | `0.5 m` | |
| MORE 최대 횟수 | `4` | |
