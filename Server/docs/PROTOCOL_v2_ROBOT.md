...
기구학적으로는 정상(제자리 회전에 가까운 선회)이지만, **모터 드라이버가 좌우 반대 방향
동시 구동을 처리하는지 확인이 필요하다** (§11).
...
- `R_draw < 0.155m`인 `ARC`가 도면에 들어오면 → 서버가 도면을 거부하고
  Qt에 `DRAW_FAIL{stage:"plan", reason:"arc_too_tight", msg:"최소 도색 반지름 0.155m"}`.
- `R_draw`가 0.155m에 가까울수록 `R_robot → 0`이라 사실상 제자리 회전이 된다.
  실사용 하한은 여유를 둬서 정할 것 (§11).
...
이것은 끌리는 펜의 기하학적 귀결이라 명령 하나로는 없앨 수 없다 — 펜이 접선 연속으로
직선→호를 지나게 하려면 로봇은 순수 원호가 아닌 궤적을 그려야 한다. 원호 도색의
실사용 반지름이 정해지면 (a) 그냥 감수, (b) 짧은 전이 구간 삽입 중에서 고를 것.
...
| `k == 0` 이고 `phase == "draw"` | **ALIGN 판정** (첫 path op의 heading으로) | 2초 |
| `op[k]`가 `role=path`인 `arc` 이고 위에 안 걸림 | **ALIGN 판정** (진입 접선 방향으로) | 2초 |
...
| 최소 도색 반지름 | `0.155m` 미만은 물리적으로 불가 → 도면 거부 (§5.5) |
...
2. 🔴 **호 진입 자세 — 꺾임이 아니라 "원이 통째로 어긋나는" 문제였다** (§5.6 개정).
   서버 구현 후 드라이런으로 실측했다 (`draw_test --arc 0.5` + `robot_sim`).

   | 항목 | 결과 |
   |---|---|
   | 서버가 보낸 `radius_m` | `0.4754` (= `sqrt(0.5² − 0.155²)`) ✅ 공식대로 |
   | 펜이 그린 호의 반지름 | `0.4996` ✅ **도면 0.5m와 일치 — §5.4 공식은 옳다** |
   | 펜 도착점 | `(0.310, −0.950)` — 도면상 `(0, −1.0)` 대비 **0.31m 어긋남** ❌ |

   반지름은 맞는데 **원의 위치가 틀렸다.** 원인: §5.3의 `move(+a)` 규칙은 직선용이라
   호 진입에 쓰면 ICR이 도면 원의 중심에 안 놓인다. 펜은 시작점에 정확히 놓이지만
   차체가 접선을 향하고 있어, 그려지는 원 전체가 `atan(d/R_robot)`(= 0.5m에서 **18.06°**)
   만큼 회전한 자리에 그려진다. §5.6이 "접합부 꺾임 18°"로 적어둔 것과 같은 각도인데,
   실제 영향은 **도착점 0.31m 이탈**이라 감수할 수준이 아니다.

   **해법 (서버만 고치면 된다. 전선 형식·로봇 동작은 그대로).**
   도색 호 앞뒤의 오프셋 보정을 이렇게 바꾼다:

   ```
   φ = atan(d / R_robot)              // 우회전 호면 오른쪽으로, 좌회전이면 왼쪽으로
   진입: turn(∓φ) → move(+a) → nozzle(down) → arc
   이탈: nozzle(up) → move(−a) → turn(±φ)   // 차체를 도면 접선으로 되돌린다
   ```

   유도: 펜 P를 도면 원의 시작점에 두고 ICR을 도면 원 중심 O에 놓으려면, 중심 C는
   `|C−P| = d`, `|C−O| = R_robot`, `OC ⊥ CP`를 만족해야 한다. `R_robot² + d² = R_paint²`
   이므로 이 삼각형은 항상 성립하고, 접선 기준 국소좌표에서 `C = (d·R_robot/R_paint, −d²/R_paint)`
   → 진입 방위는 접선에서 `atan2(−d, R_robot) = −atan(d/R_robot)`만큼 틀어진 방향이다.
   `MORE` 판정의 `exitHeadingDeg`도 접선이 아니라 `접선 ± φ`로 잡아야 한다.

   ⚠️ 이 항목이 정리되기 전까지 **호 도색은 쓰지 말 것.** 직선 경로는 영향 없다
   (드라이런에서 사각형 도면은 펜 자취가 꼭짓점과 mm 단위로 일치했다).

3. **`MORE`는 노즐이 내려간 채로 실행된다.** boundary가 `nozzle(up)` op **앞**에
   있어서, 거리 보정 주행이 그대로 도색된다. 직선에서는 이게 옳다(그리다 만 선을
   끝점까지 이어 그린다). 호에서는 접선 방향으로 직선을 덧그리게 되므로, 위 2번을
   고치기 전에는 호 뒤의 `MORE`를 끄는 편이 나을 수 있다.
4. **`R_paint` 실사용 하한**. 이론 하한은 0.155m지만 그 근처는 `R_robot → 0`이라
   사실상 제자리 회전이다. 여유 있는 운용 하한(예: 0.3m)을 실측으로 정할 것.
   `R_paint < 0.176m`에서는 안쪽 바퀴가 역회전하므로(§4.4), **모터 드라이버의 좌우
   반대방향 동시 구동 가능 여부**가 하한을 먼저 결정할 수도 있다 — 로봇팀 확인 필요.
