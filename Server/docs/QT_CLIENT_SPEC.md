...
POS 중계 중단, DRAW_DONE 추가, 관리자 창 로그인 게이트)
...
- **관리자 창(`:8083`)에 로그인 게이트가 생겼습니다** — `calib==null`일 때 안내하는
  링크를 눌러도 로그인 화면이 먼저 뜹니다. 안내 문구에 반영하세요(§3.1).
...
- ⚠️ **`calib`가 `null`이면** (아직 캘리브레이션 전) 캘리브레이션이 필요하다는 안내와 함께
  **관리자 창으로 이동하는 하이퍼링크**를 보여주세요 — 주소
  ...
  중계합니다(§4).
- 🔑 **관리자 창은 로그인 화면이 먼저 뜹니다** (2026-07-27부터). 링크를 눌러도 바로
  캘리브레이션 화면이 열리지 않고 `/login`으로 이동하며, **계정으로 로그인해야** 그
  안의 화면들이 열립니다. 캘리 결과가 "그 시점에 로그인된 계정"에 저장되는 구조라
  순서를 강제한 것입니다. 안내 문구에 이 점을 같이 적어주세요 (예: "관리자 창에서
  로그인 후 캘리브레이션을 진행하세요"). Qt가 로그인을 대신 해주거나 계정 정보를
  넘겨주는 경로는 **없습니다** — 관리자 창에서 직접 입력합니다.
...
- ⚠️ **`CALIB_START`(캘리브레이션 시작)는 QT가 보내지 않습니다** (2026-07-23 변경). 카메라
  설치/캘리브레이션은 **관리자 창(admin_console, `http://192.168.0.8:8083` — 로그인 필요)에서 시작**합니다.
...
4. 관리자 창(`http://192.168.0.8:8083/logs`)에서 `[tap] QT->SRV ...`가 흐르면 OK.
   **로그인 후에만 보입니다** — 서버팀에서 테스트 계정을 받아 로그인하세요(기본
   테스트 계정은 `test`/`1234`, `Server/tools/seed_user.py`로 생성).
...
- [ ] `calib`가 `null`이면 관리자 창 하이퍼링크(`http://192.168.0.8:8083`, Qt 고정값) 안내
      + **"관리자 창에서 로그인 후 캘리브레이션" 문구** 포함 (§3.1 — 링크만 주면 사용자가
      로그인 화면을 만나고 당황합니다)
