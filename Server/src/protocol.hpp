...
//     + 수동 노즐 NOZZLE_DOWN/NOZZLE_UP (별칭 PAINT_ON/PAINT_OFF)
//     ⚠️ 수동 조작과 수동 노즐은 **경로 실행 중(planActive_)에는 무시된다**
//       (2026-08-10). 자동 도색의 노즐 상태는 buildDrawOps가 paint + 펜
//       오프셋으로 단독 결정하므로, 중간에 수동 명령이 서보를 뒤집으면 다음
//       nozzle op이 올 때까지 반대 상태로 그린다. 점검용 탈출구는 관리자 창
//       (ADMIN)에 남아 있다 - 그 경로는 차단 판정을 거치지 않는다.
//     ⚠️ 조이스틱 명령은 서버를 "수동 모드"로 바꿔 자동 판정(ALIGN/MORE/
//       DRIFT)을 멈춘다. 노즐 명령은 로봇을 움직이지 않으므로 바꾸지 않는다.
//       수동 모드는 **새 BLUEPRINT 또는 START_DRAW**에서 풀린다 (2026-08-10:
//       예전엔 BLUEPRINT뿐이라, 도면을 올린 뒤 로봇 위치를 잡고 시작하면 그
//       작업 전체가 피드백 없이 돌았다).
