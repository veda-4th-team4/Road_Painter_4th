...
- **ALIGN 이후 READY는 즉답하지 않는다** (2026-08-03): 그 READY 수신 시점부터 새 `POS` 3장을 받은 뒤 판정하고, 1초 안에 안 차면 현재 pose로 판정한다. 미세회전(0.1~0.3초)이 CCTV→pose 반영 지연보다 짧아 회전 전 각도로 같은 ALIGN을 재발사하던 진동을 막기 위함 — 로봇 입장에선 응답이 `POS` 정상 시 100~200ms, `POS`가 끊기면 최악 1.5초 늦어질 뿐이라 **READY에 자체 타임아웃을 걸어 임의 출발하면 안 된다**. 자세한 근거는 [server_PROTOCOL.md](server_PROTOCOL.md) "ALIGN 이후 READY는 서버가 늦게 답한다". 유예 중 모인 `POS`는 버리지 않고 **theta 평균**(sin/cos 누적)에 써서 노이즈를 1/√N로 줄인다 — 추가 지연 0
- **정렬 허용 오차 2° → 4°** (2026-08-03): 정렬 루프의 1회 오차가 임계값과 같은 크기라 수렴이 불가능했다. 로봇 메인 루프 80ms × 400 SPS = **틱당 1.42° 오버슛**(한쪽 방향 계통 편향) + 평활화 없는 `theta` 노이즈(마커 13cm 기준 코너 1px당 약 0.7°). 로봇이 감속 접근/예측 정지로 오버슛을 줄이면 다시 내릴 것
- **POS 이상치 게이트** (2026-08-03, 2026-08-04 보강): 각도 변화가 `3° + 40°/s × 경과시간`(**상한 45°**)을 넘는 프레임은 폐기(연속 5회 초과 시 재동기). 로봇의 물리적 최대 회전속도가 17.8°/s로 알려져 있어, 그보다 빠른 변화는 코너 순서 회전(θ가 90°/180° 점프)이나 부분 가림 같은 **검출 오류**다. 상한을 둔 이유는 현장 `POS`가 1~2Hz라 속도항만으로는 간격이 벌어질수록 게이트가 무한정 헐거워지기 때문
- **코너 순서 뒤집힘 게이트** (2026-08-04): 각도가 60° 이상 변했는데 중심은 3cm도 안 움직인 프레임은 별도 폐기. pose 중심은 코너 4점의 **평균**이라 코너 순서가 돌아가도 안 변하는 반면 θ는 90°씩 튀므로, 이 조합은 물리적으로 불가능하고 **코너 순서 회전의 지문**이다
- **유예 중 새 POS 0장이면 GO** (2026-08-04): 유예가 끝났는데 새 `POS`가 하나도 없으면 `pose_`가 직전과 글자 그대로 같아 **똑같은 ALIGN이 또 나가는 것이 보장**된다(현장 로그: `-34.6°` 3회 반복 → 로봇이 약 104° 회전). 남은 오차는 `DRIFT`가 잡지만 중복 ALIGN의 누적 오버슛은 되돌릴 수 없어서 `GO`를 택했다
- **POS 통계 로그** (2026-08-04): 10초마다 `수신 / 채택(Hz) / 파싱실패 / 속도게이트 / 코너뒤집힘`을 한 줄 요약. 예전엔 POS가 초당 몇 장 들어오는지 서버 로그로 알 방법이 아예 없었다. `DRIFT` 주기도 200 → **400ms**(현장 POS가 1~2Hz라 5Hz 상한이 무의미했음)
...
| [docs/ROBOT_ACTION_ITEMS_20260803.md](docs/ROBOT_ACTION_ITEMS_20260803.md) | **로봇팀 전달 사항** — DRIFT 부호 반전, READY 자체 타임아웃 금지 등 필수 준수사항 |
...
│   ├── router_align.cpp    출발 전 정렬 판정 (READY → ALIGN/GO, ALIGN 반영 유예)
│   ├── router_calib.cpp    로그인 / 캘리브레이션 번들 / 채널 전환
│   ├── router_path.cpp     경로 생성·전송·복귀 (접근 → 도색 2단계, 이탈 재계획)
