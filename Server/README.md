...
- **ALIGN 이후 READY는 즉답하지 않는다** (2026-08-03, 08-04 개정): 그 READY 수신 시점부터 **2초를 고정으로 기다린 뒤** 판정한다. 미세회전(0.1~0.3초)이 CCTV→pose 반영 지연보다 짧아 회전 전 각도로 같은 ALIGN을 재발사하던 진동을 막기 위함 — 로봇 입장에선 응답이 **최악 2.5초**까지 늦어질 뿐이라 **READY에 자체 타임아웃을 걸어 임의 출발하면 안 된다**. 대기 중 모인 `POS`는 버리지 않고 **theta 평균**(sin/cos 누적)에 써서 노이즈를 1/√N로 줄인다 — 추가 지연 0. **2초 동안 새 `POS`가 한 장도 없으면 같은 ALIGN을 반복하지 않고 `GO`로 빠진다**(pose가 직전과 동일해 재판정해도 같은 값이 나오는 것이 보장되므로). 초기 구현의 "새 `POS` 3장 or 1초"는 현장 `POS`가 1~2Hz라 거의 항상 타임아웃에 먼저 걸려 폐기했다. 자세한 근거는 [server_PROTOCOL.md](server_PROTOCOL.md) "ALIGN 이후 READY는 서버가 늦게 답한다"
- **도색 진입 시 NOZZLE 앞에서도 정렬한다** (2026-08-04, 로봇팀 요청): 로봇이 `phase=draw` PATH를 받자마자 첫 세그먼트(`NOZZLE`) 실행 전에 `READY {"seg":0}`을 보내고, 서버가 여기에도 ALIGN/GO를 준다. 후방 노즐 오프셋 보정용 **+15.5cm 사전 전진** 방향이 첫 도색 방향과 어긋나면 노즐이 시작점 정중앙에 안 떨어지기 때문. `NOZZLE`에는 `heading_deg`가 없을 수 있어, 서버는 **뒤따르는 세그먼트에서 처음 나오는 `heading_deg`를 목표로 삼는다**
- **DRIFT 주기 5Hz → 2.5Hz** (2026-08-04): 현장 `POS` 실측이 1~2Hz라 200ms 상한은 애초에 병목이 아니었다. 응답성 손해 없이 로봇 쪽 로그·UART 부담만 줄어든다
...
- **POS 이상치 게이트** (2026-08-03): 각도 변화가 `3° + 40°/s × 경과시간`을 넘는 프레임은 폐기(연속 5회 초과 시 재동기). 로봇의 물리적 최대 회전속도가 17.8°/s로 알려져 있어, 그보다 빠른 변화는 코너 순서 회전(θ가 90°/180° 점프)이나 부분 가림 같은 **검출 오류**다
