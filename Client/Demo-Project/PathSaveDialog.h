...
public:
    explicit PathSaveDialog(QWidget *parent = nullptr) : QDialog(parent) {
        setWindowTitle("경로 저장 설정");
        setFixedSize(350, 150);

        QLabel *lblMessage = new QLabel("경로 지정이 완료되었습니다.\n데이터를 어떻게 저장하시겠습니까?", this);
        lblMessage->setAlignment(Qt::AlignCenter);

        btnSaveTemp = new QPushButton("저장하기\n(path_temp.json)", this);
        btnExportFile = new QPushButton("파일로\n내보내기...", this);
        btnCalibrate = new QPushButton("Top View 보정\n(호모그래피 적용)", this); // 💡 새로 추가된 버튼
        ...
        connect(btnCalibrate, &QPushButton::clicked, this, [this]() { done(3); });     // 💡 3: 호모그래피 보정 저장
