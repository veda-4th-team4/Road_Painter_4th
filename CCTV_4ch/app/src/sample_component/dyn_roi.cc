...
const int   kDefaultMargin   = 240;  // operator's upper bound at boot (px)
...
DynRoiTracker::DynRoiTracker()
    : enabled_(false), tracking_(false), miss_(0),
...
void DynRoiTracker::report() const
{
    ...
    snprintf(json, sizeof(json),
             "{\"type\":\"DYNROI_STATE\",\"tracking\":%d,"
             "\"x\":%d,\"y\":%d,\"w\":%d,\"h\":%d,"
             "\"margin_used\":%d,\"marker_px\":%.1f,\"motion_px\":%.1f}",
             tracking_ ? 1 : 0, roi_.x, roi_.y, roi_.width, roi_.height,
