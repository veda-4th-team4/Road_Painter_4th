...
//    - 예: 10cm(0.1m) 명령 시 실측이 9.5cm면 FACTOR = 0.1m / 0.095m = 1.053f 로
//    수정
constexpr float DISTANCE_CALIB_FACTOR = 1.010f;

// 2. 회전 각도 보정 계수 (Turn Angle Scaling Factor)
//    - 예: 90도 회전 명령 시 실측이 95도 회전하면 FACTOR = 90.0f / 95.0f =
//    0.947f 로 수정
constexpr float TURN_ANGLE_CALIB_FACTOR = 1.030f;
...
uint32_t CalculateMoveSteps(float dist_m) {
  ...
  float steps_per_meter =
      (STEPS_PER_REV * GEAR_RATIO) / wheel_circ; // ~15433.09 steps/m
...
uint32_t CalculateTurnSteps(float angle_deg) {
  ...
  float arc_len =
      turn_circ * (std::fabs(angle_deg) * TURN_ANGLE_CALIB_FACTOR / 360.0f);
...
void PrintHelp() {
  ...
  std::cout << "  move <dist_m>      : Straight move in meters (e.g. 'move "
               "0.1' = 10cm forward, 'move -0.1' = 10cm reverse)\n";
  std::cout << "  turn <angle_deg>   : Turn in degrees (e.g. 'turn 20' = 20 "
               "deg CW right, 'turn -20' = 20 deg CCW left)\n";
  std::cout << "  nozzle <1|0>       : Control nozzle actuator (1=DOWN/ON, "
               "0=UP/OFF)\n";
  std::cout
      << "  status             : Read latest telemetry status from STM32\n";
...
int main(int argc, char **argv) {
  ...
  std::cout << GetTimestampStr()
            << "[CALIB] Successfully connected to STM32 via " << device
            << std::endl;
  std::cout << GetTimestampStr() << "[CALIB] Current Calibration Factors:\n";
  std::cout << "         - DISTANCE_CALIB_FACTOR   : " << DISTANCE_CALIB_FACTOR
            << "\n";
  std::cout << "         - TURN_ANGLE_CALIB_FACTOR : "
            << TURN_ANGLE_CALIB_FACTOR << "\n";
  std::cout << "         - Wheel Diameter          : "
            << WHEEL_DIAMETER_M * 1000.0f << " mm\n";
  std::cout << "         - Wheelbase               : " << WHEELBASE_M * 1000.0f
            << " mm\n";
  ...
  while (true) {
    ...
    if (cmd == "quit" || cmd == "exit") {
      std::cout << GetTimestampStr() << "[CALIB] Exiting calibration tool."
                << std::endl;
      break;
    } else if (cmd == "help") {
      PrintHelp();
      ...
      if (robot_comm.GetLatestStatus(status)) {
        std::cout << GetTimestampStr() << "[STATUS] Left Steps: "
                  ...
                  << std::hex << (int)status.flags << std::dec << std::endl;
      } else {
        std::cout << GetTimestampStr()
      ...
      float dist_m = 0.0f;
      if (!(iss >> dist_m)) {
        std::cout << "[CALIB] Usage: move <dist_m> (e.g. move 0.1)"
                  << std::endl;
        continue;
      }
      ...
      if (!robot_comm.GetLatestStatus(start_status)) {
        std::cout << GetTimestampStr()
                  << "[CALIB] Warning: STM32 telemetry offline. Using 0 step "
                     "baseline."
                  << std::endl;
      }
      ...
      while (!completed) {
        ...
        if (elapsed_ms > 30000) { // 30s safety timeout
          std::cout << GetTimestampStr()
                    << "[CALIB MOVE] Timeout (30s) reached!" << std::endl;
          break;
        }
      ...
      int32_t final_l =
          std::abs(static_cast<int32_t>(end_status.left_steps) - start_l);
      int32_t final_r =
          std::abs(static_cast<int32_t>(end_status.right_steps) - start_r);
      uint32_t final_avg = (final_l + final_r) / 2;

      std::cout << GetTimestampStr() << "===== [CALIB MOVE RESULT] =====\n";
      std::cout << "  Requested Dist: " << dist_m << " m (" << (dist_m * 100.0f)
                << " cm)\n";
      std::cout << "  Target Steps  : " << target_steps << "\n";
      std::cout << "  Actual Steps  : Left=" << final_l << ", Right=" << final_r
                << " (Avg=" << final_avg << ")\n";
      std::cout << "  Step Delta    : " << (final_avg - target_steps)
                << " steps\n";
      std::cout << "=================================\n";
      ...
      if (!(iss >> angle_deg)) {
        std::cout << "[CALIB] Usage: turn <angle_deg> (e.g. turn 20)"
                  << std::endl;
        continue;
      }
      ...
      // Protocol v2 sign convention: positive angle = turn right (CW) -> Left
      ...
      while (!completed) {
        ...
        if (elapsed_ms > 30000) {
          std::cout << GetTimestampStr()
                    << "[CALIB TURN] Timeout (30s) reached!" << std::endl;
          break;
        }
        usleep(20000);
      }
      ...
      std::cout << "  Target Steps   : " << target_steps << "\n";
      std::cout << "  Actual Steps   : Left=" << final_l
                << ", Right=" << final_r << " (Avg=" << final_avg << ")\n";
      std::cout << "  Step Delta     : " << (final_avg - target_steps)
                << " steps\n";
      std::cout << "=================================\n";
    } else {
      std::cout << "[CALIB] Unknown command: " << cmd
