...
IntrinsicsCalib::IntrinsicsCalib()
  ...
  for (int c = 0; c < kChannels; ++c) {
    ...
    S.capture_deadline_ms = 0;
    S.rms = 0.0;
    S.pruned_views = 0;
    S.last_capture_ms = 0;
    S.probe_total = 0;
    S.probe_ms = 0;
    S.recent_corners.clear();
    S.recent_markers.clear();
...
bool IntrinsicsCalib::ProbeIfDue(int ch, const cv::Mat& gray, long now_ms, long hold_ms) {
  if (!ValidCh(ch)) return false;
  Session& S = sess_[ch];
  if (S.state != CS_COLLECTING) return false;
  if (S.probe_ms != 0 && now_ms - S.probe_ms < CALIB_PROBE_MS) return false;

  std::vector<cv::Point2f> corners;
  std::vector<int> ids;
  std::vector<cv::Point2f> mcen;
  std::vector<int> mids;
  DetectBoard(gray, corners, ids, &mcen, &mids);
  S.probe_total = (board_.squares_x - 1) * (board_.squares_y - 1);
  S.probe_ms = now_ms;  // a probe ran, whether or not it found anything

  // Flicker hold, DISPLAY ONLY, per corner/marker id — see Session::RecentPoint's
  // doc comment for why "what did THIS attempt see" is the wrong question at
  // this board's print size. Fold this attempt's sightings into the caches,
  // then rebuild the viewfinder as everything still within hold_ms — not just
  // this one attempt. Whatever CAPTURE actually banks is still judged fresh,
  // per frame, in CaptureView() — none of this touches that.
  for (size_t i = 0; i < ids.size() && i < corners.size(); ++i)
    S.recent_corners[ids[i]] = Session::RecentPoint{corners[i], now_ms};
  for (size_t i = 0; i < mids.size() && i < mcen.size(); ++i)
    S.recent_markers[mids[i]] = Session::RecentPoint{mcen[i], now_ms};

  S.probe_corners.clear();
  S.probe_ids.clear();
  for (std::map<int, Session::RecentPoint>::iterator it = S.recent_corners.begin();
       it != S.recent_corners.end(); ) {
    if (now_ms - it->second.seen_ms > hold_ms) {
      S.recent_corners.erase(it++);
      continue;
    }
    S.probe_corners.push_back(it->second.pt);
    S.probe_ids.push_back(it->first);
    ++it;
  }

  std::vector<cv::Point2f> heldCen;
  std::vector<int>         heldIds;
  for (std::map<int, Session::RecentPoint>::iterator it = S.recent_markers.begin();
       it != S.recent_markers.end(); ) {
    if (now_ms - it->second.seen_ms > hold_ms) {
      S.recent_markers.erase(it++);
      continue;
    }
    heldCen.push_back(it->second.pt);
    heldIds.push_back(it->first);
    ++it;
  }
  // Judge the layout on the same accumulated set the viewfinder is showing,
  // so the two can never disagree about what the operator sees on screen.
  S.board_fit_rms   = MarkerLayoutRms(heldCen, heldIds, board_.squares_x, board_.squares_y);
  S.board_fit_rms_t = MarkerLayoutRms(heldCen, heldIds, board_.squares_y, board_.squares_x);
  return true;
}
...
bool IntrinsicsCalib::RequestCapture(int ch, const char** reason) {
  // ch < 0 arms EVERY open session, so one press banks one board pose into
  // every lens that can see it. A lens that cannot is not an error here — it
  // refuses on its own frame, with its own reason, which is what the per-lens
  // readout is for.
  //
  // ch >= 0 arms only that lens. See the header for why that is now the form
  // the dashboard sends: the all-lenses press quietly fed a stale session's
  // K with board scraps for a whole afternoon.
  //
  // Every armed session shares ONE deadline (computed once, not per lens) so
  // that "the same board pose" keeps meaning what it always did: every lens's
  // retry burst ends at the same instant, not whichever instant its own
  // frames happened to arrive on.
  if (ch >= 0 && !ValidCh(ch)) {
    if (reason) *reason = "채널 번호가 0..3 범위를 벗어났습니다";
    return false;
  }
  const long long deadline = monotonic_ms() + CALIB_CAPTURE_BURST_MS;
  int armed = 0;
  for (int c = 0; c < kChannels; ++c) {
    if (ch >= 0 && c != ch) continue;
    if (sess_[c].state != CS_COLLECTING) continue;
    sess_[c].capture_pending = true;
    sess_[c].capture_deadline_ms = deadline;
    ++armed;
  }
  if (armed == 0) {
    // Distinguished, because the two have different fixes and the operator
    // cannot see session state from the capture button: one means "start a
    // session", the other means "you picked the wrong lens in the dropdown".
    if (reason) {
      *reason = (ch >= 0) ? "이 렌즈에 열린 세션이 없습니다 — 드롭다운 채널을 확인하거나 세션을 시작하세요"
                          : "열린 세션이 없습니다 — 먼저 렌즈를 골라 세션을 시작하세요";
    }
    return false;
  }
  if (reason) *reason = NULL;
...
CalibState IntrinsicsCalib::TakePendingCapture(int ch, const cv::Mat& gray) {
  ...
  S.probe_ms = 0;
  // Keep retrying on the lens's OWN next incoming frame until either this
  // pose is accepted or the shared burst deadline passes — see
  // CALIB_CAPTURE_BURST_MS. A rejection here is one unlucky frame (a hand
  // still settling, a corner blurred mid-refinement), not a reason to make
  // the operator notice and press the button again: accuracy matters more
  // than answering on the very first frame, and nothing about this path is
  // real-time. CaptureView()'s own gates (sharpness, coverage, corner count,
  // pose diversity, min-gap-since-last-ACCEPTED-capture) are untouched by
  // this — a genuinely bad view still gets refused every one of the retries.
  if (S.last_capture == CS_CAPTURED || monotonic_ms() >= S.capture_deadline_ms)
    S.capture_pending = false;
